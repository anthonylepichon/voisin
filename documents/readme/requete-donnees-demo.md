...
Description générale : Documente le jeu de données local de démonstration du projet Voisin et contient la requête SQL complète.
Rôle : Expliquer les comptes fictifs et fournir le script SQL qui peuple toutes les tables métier.
Tâches : Identifier les profils, le mot de passe commun local, les volumes insérés et la requête SQL à importer.
Liens avec les autres fichiers : Utilise les images de démonstration présentes dans uploads/profils.
-->

# Requête des données de démonstration

La requête SQL complète à importer est intégrée à la fin de ce document. Elle est prévue pour une base locale `voisin` à jour avec toutes les migrations Doctrine du projet. Elle n'est jamais importée en production.
